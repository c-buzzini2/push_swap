...
/*   Updated: 2025/02/11 11:27:40 by cbuzzini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

static int	ft_calculate_cost(int index_a, int index_b,
...
static t_list	*ft_smallest_higher(t_list *tail_a, t_list *curr, int nb)
{
...
static t_list	*ft_find_destination(t_list *tail_a, int nb)
{
	t_list	*curr;
...
t_list	*ft_choose_cheapest(t_list *tail_a, t_list *tail_b)
{
	...
	while (lstsize_b > 0)
	{
		...
		cost = ft_calculate_cost(curr->dest->index, curr->index,
				tail_a, tail_b);
		if (cost < lowest_cost)
		{
			lowest_cost = cost;
